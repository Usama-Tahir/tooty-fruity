...
async function isAuthenticated(req, res, next) {
  try {
    ...
    if (user && user.type === 'tutor' && !user.isZoomAccount && process.env.PLATFORM_ONLINE === PLATFORM_ONLINE.ZOOM_US) {
      return next(PopulateResponse.unauthenticated());
    }
...
exports.middleware = {
  ...
  canAccessFeature(userType) {
    if (!userType) {
      throw new Error('Required user type needs to be set');
    }

    return compose()
      .use(isAuthenticated)
      .use((req, res, next) => {
        if (req.user.role === 'admin' || req.user.type === userType) {
          return next();
        }
